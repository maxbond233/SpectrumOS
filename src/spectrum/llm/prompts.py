...
[
  {
    ...
    "tags": ["统计学/降维", "机器学习/无监督学习"],
    "links": [
      {"target": "t-SNE", "relation": "对比", "note": "线性 vs 非线性降维"},
      {"target": "特征值分解", "relation": "前置", "note": "PCA 的数学基础"}
    ]
  }
]
...
🏷️ 标签规则：
- tags 是字符串数组，用 "/" 分隔层级（如 "机器学习/强化学习/RLHF"）
- 最多 3 级层级，每张卡片 1-3 个标签
- 标签应反映概念所属的知识领域和分类体系

🔗 概念链接规则：
- links 是对象数组，每个包含 target、relation、note
- target 必须精确匹配本批次中其他卡片的 concept 字段
- relation 可选值：相关/前置/演进/对比/包含
- note 简要说明链接原因（一句话）
- 只链接本批次内存在的概念，不要链接不存在的概念
"""
